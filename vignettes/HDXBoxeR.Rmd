...
vignette: |
  %\VignetteEngine{knitr::rmarkdown} %\VignetteIndexEntry{HDXBoxeR} %\VignetteEncoding{UTF-8}
---
```{r, include = FALSE}
...
10.	Cheatsheet aka section for impatient. 
...
HDXBoxeR significantly facilitates the comparison of multiple different Protein States in the time series and number of replicates. We recommend using at least 3 replicates to have good statistical analysis. All the Protein States imported are required to have the same number of replicates. Another recommendation is to use only peptides the User is confident of. There is no point in analyzing bad data. 

The example looks as follows:

![](figures/example_table.png)
 
The input file has 24 columns including the “Mark” column. We keep typically just peptides with high and medium confidence. Example data compares two Protein States: Unbound and Bound. Each of the sets has three replicates.  Data also should have one replicate of undeuterated and fully deuterated sets labeled as 0s and FD respectively for allowing full analysis. 

## 2.	Installation
Install HDXBoxeR package by using command: install.packages(HDXBoxeR). The package is deposited in GIT in CRAN.

## 3.	How to load data into R

To load data into R you will need input as described in section 1 (file should have 24 columns!). Example input is provided as a reference and to start the fun. 
Data, when it is loaded, will be reprocessed to different formats depending on what kind of analysis will be performed. 

There are 4 possible options in which input can be processed as listed below. Function names reflect which option is chosen.
Input can be formatted to:
  
* Analyze time points **or** time courses analysis (tp vs tcourse options in the functions).
* Analyze Deuterons Uptake **or** Percent Deuteration (uptake no additional label, percent with proc)
* Use all protein states **or** use selected (option states in the name of a function to select specific protein states).
...
To compare uptake data for two different protein states output_tp() function will be used. If one would want to use percent data output_tp_proc() should be used. However, if the time course series for percent data is of interest output_tcourse_proc() function should be used. Finally if one would like to have the most complex option which would be an analysis of percent data for time courses format without matching peptide sequences and for selected protein states one would use output_tcourse_noSeq_states_proc() function. 

In principle most often used are functions output_tp(), output_tcourse() and their percent deuteration counterparts: output_tp_proc(), output_tcourse_proc()
...
b<-output_tp_proc(file_nm) 

#################
####Time courses vs time points 
###uptake
#Time point comparisons 
a<-output_tp(file_nm) 
# Time courses comparisons
b<-output_tcourse(file_nm) 
###procent deuteration
d<-output_tp_proc(file_nm) 
# Time courses comparisons
e<-output_tcourse_proc(file_nm)

#################
####Analyze selected states
names_states<- nm_states(file_nm) ## returns names of the states in file.
h<-output_tp_states(file_nm, states=names_states[1] )
i<-output_tcourse_state(file_nm, state=names_states[1] )
##other possible functions to use would be output_tp_proc_state() & output_tcourse_proc_state()

#################
####Peptide sequence matching between two states
##if there is no specification for not matching all sequences have to indentical (use functions shown above)
## No peptide sequence matching function
j<-output_tp_noSeq(file_nm)

## other functions from this family:
#  output_tp_noSeq_proc(), output_tp_noSeq_states(), output_tp_noSeq_states_proc(), 
#  output_tcourse_noSeq(), output_tcourse_noSeq_states(),output_tcourse_noSeq_proc() output_tcourse_noSeq_states_proc()
...
HDXBoxeR provides a statistical framework to identify peptides that are significantly different between given protein states. To be able to do statistics we recommend providing 3 replicates for each protein state. **Number of replicates have to be the same for all Protein States in the analysis.**  
...
h<-output_tp_states(file_nm, states=names_states[c(2,1)] )
pv2<-pv_timepoint(h)
head(pv2)
```
...
If the plots that are return have chopped labels adjust the fugure sizes. 

### Boxplots
Boxplots allows for fast comparisons of average uptake or percent deuteration for all protein states.
...
### average plots for procent deuteration per peptide
plots_av_tp(b)
...
d<-output_tp_states(file_nm, states = rev(names_states))
...
![](figures/max_uptake_ave_uptake.png)
...
### heat maps 
plot_heat_map_tp(a, replicates=3)
...
### 
plot_heat_map_max_uptake_tp_proc(input_up = a, input_proc = b, replicates=3, mar_x=8, ranges = c(-Inf, -10, -2.5,0,2.5, 10, Inf), pv_cutoff=0.01)
```
...
tc_aP<-output_tcourse_proc(filepath = file_nm)
...
file_nm<-system.file("extdata", "All_results_table.csv", package = "HDXBoxeR") 
## input for timecourses for procent deuteration and uptake
tc_a<-output_tcourse(filepath = file_nm)
tc_aP<-output_tcourse_proc(filepath = file_nm)
...
```{r echo = FALSE, fig10a, fig.height = 4, fig.width = 3}
file_nm<-system.file("extdata", "All_results_table.csv", package = "HDXBoxeR") 
## input for deuteration uptake
a<-output_tp(file_nm) 
# input for procent deuteration
...
pymol_script_significant_peptide(a, ranges=c(-Inf, seq(-120, 120, by=10), Inf), pv_cutoff = 0.005,replicates = 3)
...
output_tp_csv(filepath =file_nm , output = "output_test.csv")
## for percent deuteration for time points use function output_tp_proc_csv()
# for deuteration uptake/procent deuteration for time courses use functions: output_tcourse_csv(), output_tcourse_proc_csv() respectively
...
## for procent deuteration use function verbose_timepoint_output_proc()
## use verbose_timepoint_output_subset() to select few states. 
```
