####################
#' Prepares output for HDX-MS for the timecourses without matched peptide sequences.
#'
#' Returns a dataframe organized for additional analysis.
#' Function does not match for the sequence. It can be used to compare mutant sequences.
...
#' @export
output_tcourse_noSeq<- function(filepath){
  Start<-c()
  End<-c()
  Charge<-c()
...
#' @examples
#' file_nm<-system.file("extdata", "All_results_table.csv", package = "HDXBoxeR")
#' a<- output_tcourse_noSeq_proc(file_nm)
#' @export
output_tcourse_noSeq_proc<- function(filepath){
  Start<-c()
  End<-c()
  Charge<-c()
  a<-read.csv(file=filepath,  header = F, skip = 1)### load the file witout headers
...
#' Prepares output for HDX-MS for the time courses without matching sequences for selected states.
#'
#' Returns a dataframe organized for additional analysis.
#' Function does not match for the sequence. It can be used to match mutant sequences.
...
#' a<- output_tcourse_noSeq_states(file_nm, states=names_states)
#' @export
output_tcourse_noSeq_states<- function(filepath, states){
  Start<-c()
  End<-c()
  Charge<-c()
  a<-read.csv(file=filepath,  header = F, skip = 1)### load the file witout headers
...
#' file_nm<-system.file("extdata", "All_results_table.csv", package = "HDXBoxeR")
#' names_states<- nm_states(file_nm)
#' a<- output_tcourse_noSeq_states_proc(file_nm, states=names_states)
#' @export
output_tcourse_noSeq_states_proc<- function(filepath, states){
  Start<-c()
  End<-c()
  Charge<-c()
  a<-read.csv(file=filepath,  header = F, skip = 1)### load the file witout headers
